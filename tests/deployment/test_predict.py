...
def test_prepare_data(video):
    ...
    selected_columns =['index','channelId','videoCategoryId', 'channelViewCount',
                    ...
                    'channelCommentCount', 'videoViewCount','elapsedtime', 'videoDislikeCount',
                    'videoPublished', 'VideoCommentCount']
    
    df.expect_table_columns_to_match_ordered_list(column_list=selected_columns)  # schema adherence

    df.expect_column_values_to_be_between(column='videoViewCount',
                                          min_value=0,strict_min=True) #positive values
    df.expect_column_values_to_be_between(column='videoDislikeCount',
                                          min_value=0,strict_min=True) #positive values
    df.expect_column_values_to_be_between(column='VideoCommentCount',
                                          min_value=0,strict_min=True) #positive values

    df.expect_column_values_to_be_of_type(column="channelId", type_="category") # type adherence
